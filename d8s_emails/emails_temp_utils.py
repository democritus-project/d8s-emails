...
def list_delete_empty_items(list_arg: list) -> list:
    ...
    empty_values: tuple = ("", [], 0, False, None)
...
def dict_add(dictionary: Dict[Any, List[Any]], key: Any, value: Any) -> Dict[Any, List[Any]]:
    """Add the given value to the dictionary at the given key.

    This function expects that all values of the dictionary parameter are lists.
    """
    if key in dictionary:
        if not isinstance(dictionary[key], list):
            message = (
                f'The value at the "{key}" key in the dictionary is not a list and the dict_add function '
                "requires all values to be a list."
            )
